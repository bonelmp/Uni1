...
//Updates the word , attempts , and time on the screen
	...
	public void processInput() {
		inputField.addActionListener(e ->{
			...
			if(input.length() !=1 || !Character.isLetter(input.charAt(0))) {
				System.out.println("Bitte nurn einen Buchstabe eingeben!");
				return;
			}
	...
	protected void handleGuess(char guess) {
		if (this.guessedLetters.contains(guess)) {
			System.out.println("Der Buchstabe " + guess + " wurde schon richtig erraten");
			return;
		}
		guessedLetters.add(guess);
		if (!this.currentWord.contains(String.valueOf(guess))) {
			this.attemptsLeft = this.attemptsLeft - 1;
			System.out.println("Falscher Buchstabe! Anzahl Versuche übrig: " + this.attemptsLeft);
		}
		updateDisplay();
		if(hasWon()) {
			endGame(true);
...
//TODO Task 1: End the game and show a message
	protected void endGame(boolean won) {
		if (won) {
			System.out.println("Glückwunsch du hast " + currentWord + " richtig erraten!");
		} else {
			System.out.println("Leider hast du das Wort nicht erraten können...");
		}
		//Geht auch eleganter mit Fragezeichen Operator
